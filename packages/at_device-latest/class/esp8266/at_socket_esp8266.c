...
static int esp8266_socket_close(struct at_socket *socket)
{
    ...
    if (resp)
    {
        ...
        /* workaround: if we have UNLINK that means we tried to close something already closed */
        if (!strncmp("UNLINK",resp->buf,MIN(6,resp->buf_len)))
        {
            result = RT_EOK;
        }
        at_delete_resp(resp);
