...
void pcf8574_pin_write(pcf8574_device_t dev, uint8_t bit, uint8_t value)
{
    ...
    if (value == 0)
        dev->port_value &= ~(1 << bit);
    else 
        dev->port_value |= 1 << bit;

    read_value = pcf8574_port_read(dev);
    pcf8574_port_write(dev, dev->port_value);
    read_value = pcf8574_port_read(dev);
    //RT_ASSERT(dev->port_value == read_value);
}
...
pcf8574_device_t pcf8574_init(const char *dev_name, rt_uint8_t i2c_addr)
{
    ...
    dev->port_value = 0xFF;
    rt_device_write(&dev->bus->parent, dev->i2c_addr, &(dev->port_value), 1);
